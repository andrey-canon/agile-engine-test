...
clean: test_requirements## delete generated byte code and coverage reports
	coverage erase
	rm -rf assets
	rm -rf pii_report
...
test_requirements: ## install requirements for production
...
test: clean ## run tests and generate coverage report
	coverage run --source ./apps manage.py test --settings=accounting_notebook.settings
...
quality: style isort_check lint ## check code style and import sorting, then lint
	diff-quality --violations=pylint --fail-under=100 --options="--rcfile=./setup.cfg" --compare-branch=origin/main
